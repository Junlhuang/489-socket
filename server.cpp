...
int run_server(int port, int queue_size) {
	...
	// (2) Set the "reuse port" socket option to allowing it to reuse the port
	int yesval = 1;
	...
	// make_server_sockaddr is a helper func that manually packs a sockaddr_in
	...
	while (true) {
		// accept()
		// one by one forever, so no sockaddr storage needed
